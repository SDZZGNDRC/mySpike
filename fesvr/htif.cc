...
void htif_t::start()
{
  if (!targs.empty() && targs[0] != "none")
      load_program(); // Load the program which user passed.
...
{
  std::string path;
  if (access(payload.c_str(), F_OK) == 0) // check if the file is existed.
    path = payload;
  else if (payload.find('/') == std::string::npos) //if '/' isn't in payload
  {
    std::string test_path = PREFIX TARGET_DIR + payload; //try to infer the correct path of the payload
