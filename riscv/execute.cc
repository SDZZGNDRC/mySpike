...
static void commit_log_print_insn(processor_t *p, reg_t pc, insn_t insn)
{
  ...
  // From the 'encoding.h', we know that PRV_U, PRV_S, PRV_M respectively equal to 0, 1, 3
  ...
  for (auto item : reg) {
    if (item.first == 0)
    {
      fprintf(log_file, " x0 0x0000000000000000");
      continue;
    }
