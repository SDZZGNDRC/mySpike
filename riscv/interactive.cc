...
void sim_t::interactive()
{
  ...
  while (!done())
  {
    ...
    if (!(ss >> cmd)) // if there is no cmd inputed.
    {
      set_procs_debug(true); // print the debug info
      step(1);
#ifdef HAVE_BOOST_ASIO
...
{
  size_t steps = args.size() ? atoll(args[0].c_str()) : -1; // 'size_t' expand to unsigned long.
  ctrlc_pressed = false;
