...
# define READ_MEM(addr, val, size) ({})
#else
# define READ_MEM(addr, val, size) \
  proc->state.log_mem_read.push_back(std::make_tuple(addr, val, size));
#endif
  ...
  #define load_func(type, prefix, xlate_flags) \
    inline type##_t prefix##_##type(reg_t addr, bool require_alignment = false) { \
      ...
      if ((xlate_flags) == 0 && likely(tlb_load_tag[vpn % TLB_ENTRIES] == vpn)) { \
        data = from_target(*(target_endian<type##_t>*)(tlb_data[vpn % TLB_ENTRIES].host_offset + addr)); \
